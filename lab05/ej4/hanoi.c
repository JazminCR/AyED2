...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    hanoi->target = stack_empty(); // inicializo
    hanoi->source = stack_empty(); // inicializo
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
    assert(hanoi != NULL);
    stack_destroy(hanoi->source); // destruir los stacks inicializados
    stack_destroy(hanoi->aux);
    stack_destroy(hanoi->target);
    free(hanoi);                  // liberar esto al último
    return NULL;
}
